  ...
  methods: {
    async click(event) {
      if (event === "Delete") {
        ...
        this.$store.dispatch("messages/setMessage", { message: `Remove dataset ${data.title}`});
        this.isLoad = false;
        this.dialog = false;
        return;
      }
      if (this.$refs.form.validate()) {
        ...
        if (event === "New") {
          ...
          this.$store.dispatch("messages/setMessage", { message: `Add dataset ${data.title}`});
        }
        if (event === "Edit") {
          const data = await this.$store.dispatch("datasets/edit", dataset);
          this.$store.dispatch("messages/setMessage", { message: `Edit dataset ${data.title}`});
        }
        this.isLoad = false;
        this.dialog = false;
      }
